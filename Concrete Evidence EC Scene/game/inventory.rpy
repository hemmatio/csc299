...
init python:
    ...
    def hide_all_inventory():
        ...
        renpy.show_screen("full_inventory")
                        ...
                        if item1.x <= x <= item1.x + item1.width and item1.y <= y <= item1.y + item1.height:
                            ...
                            if i_combine == False:
                                ...
                                for item3 in environment_sprites:
                                    ...
                                    if items_overlap == True:
                                        ie_overlap = True 
                                        # ---- EXAMPLE INTERACTION ---- #type1 is the item in the toolbox, and item3 is the item in the envrionment
    ...
    def checkItemsOverlap(item1, item2):
        if not hasattr(item1, 'width') or not hasattr(item2, 'width') or not hasattr(item1, 'height') or not hasattr(item2, 'height'):
            return False
...
screen full_inventory:
    ...
    imagebutton auto "UI/open-inv-%s.png" action If(renpy.get_screen("toolbox") == None, true= [Show("inv_buttons"), Show("close_inv"), Show("toolbox"), Hide("inventory"), Hide("toolboxpop")], false= [Hide("close_inv"), Hide("inv_buttons"), Hide("toolbox"), Hide("toolboxpop")]) xpos 0.0 ypos 0.36
